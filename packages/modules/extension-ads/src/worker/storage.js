...
µb.loadSelectedFilterLists = async function() {
    ...
    console.log("µb.loadSelectedFilterLists: bin=",bin)
    ...
    console.log("Select default filter lists if first-time launch: lists=",lists)
...
µb.getAvailableLists = async function() {
    ...
    // Custom filter lists
    console.log("b.getAvailableLists: this.userSettings.importedLists= ",this.userSettings.importedLists)
...
µb.listMatchesEnvironment = function(details) {
    ...
    if ( typeof details.lang === 'string' ) {
        ...
        if ( re === undefined ) {
            const match = /^[a-z]+/.exec(chrome.i18n.getUILanguage());
            //const match = /^[a-z]+/.exec('en-GB')
            if ( match !== null ) {
