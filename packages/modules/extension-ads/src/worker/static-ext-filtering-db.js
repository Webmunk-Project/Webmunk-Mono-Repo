...
const StaticExtFilteringHostnameDB = class {
    constructor(nBits, version = 0, name = 'unknown') {
        this.name = name;
    ...
    store(hn, bits, s) {
        ...
        if ( iHn === undefined ) {
            if ( hnIsNotRegex ) {
                //console.log("store: inserting "+hn)
    ...
    clear() {
        console.log("clearing StaticExtFilteringHostnameDB : "+this.name, this.hostnameToSlotIdMap.get("melty.fr"))
    ...
    fromSelfie(selfie) {
        console.log(`fromSelfie setting hostnameToSlotIdMap[${this.name}] to `,selfie.hostnameToSlotIdMap);
